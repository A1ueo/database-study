...
-- 5500원 보다 싸고, 동시에 단백질량이 25g보다 많은 버거 찾기
SELECT *
FROM burgers
WHERE price < 5500 AND protein > 25;

-- 5500원 보다 싸거나, 단백질량이 25g보다 많은 버거 찾기
SELECT *
FROM burgers
WHERE price < 5500 OR protein > 25;

-- 단백질량이 25g보다 많지 않은 버거 찾기
SELECT *
FROM burgers
WHERE NOT protein > 25; -- 혼동 방지를 위해 괄호를 명시하는 것도 좋음
-- WHERE !(protein > 25); -- MySQL에서만 작동(비표준)
-- 가독성과 이식성을 위해서 NOT을 사용하는 것이 가장 안전
...
-- 산술 연산자 활용 예
-- SELECT문에서의 산술 연산 예시
SELECT *, ROUND(price / gram * 100) AS 'price/100g' -- 따옴표("",'') 또는 백틱(``, 비표준?)으로 감싸기
FROM burgers;
...
-- 3. 모든 학생의 총점은?
SELECT id, nickname, (math + english + programming) AS '총점'
FROM students

-- 4. 모든 학생의 평균은?
SELECT id, nickname, (math + english + programming) / 3 AS '평균'
FROM students
