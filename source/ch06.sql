-- Active: 1749543480014@@127.0.0.1@3306@stargram
/*
6. 관계 만들기
6.1 관계란
*/
...
CREATE TABLE example (
    ...
    id INT NOT NULL,
    UNIQUE (id) -- (2)
    id INT NOT NULL KEY id INT NOT NULL UNIQUE
);
...
CREATE TABLE users (
    ...
    status VARCHAR(10) DEFAULT 'acitve', -- 계좌 상태(기본값은 'active');
...
CREATE TABLE capitals (
    ...
    -- 타입은 국가 테이블과 동일하게 맞춤
    -- 고유한 값만 허용(한 나라에 수도가 2개기 있으면 안됨) => UNIQUE: 일대일 관계를 만들기 위한 중요한 제약 조건
    PRIMARY KEY (id), -- 기본키 지정: id
...
FROM countries
    JOIN capitals
WHERE
    countries.id = capitals.country_id;
...
SELECT * FROM teams JOIN players WHERE team_id = teams.id;
...
CREATE TABLE settings (
  id INTEGER AUTO_INCREMENT, -- 아이디(자동으로 1씩 증가)
  private BOOLEAN NOT NULL, -- 계정 공개 여부(NULL을 허용하지 않음)
  account_suggestions BOOLEAN NOT NULL, -- 계정 추천 여부(NULL을 허용하지 않음)
