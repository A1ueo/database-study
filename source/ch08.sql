...
SELECT gender AS 성별, AVG(height) AS '평균 키', MAX(height), MIN(height)
FROM students
...
-- 특정 도시의 연도별 총 매출 집계
-- 일단 먼저 특정 도시별 총매출 집계
SELECT city AS 도시, SUM(amount) AS '총 매출'
FROM sales
GROUP BY city;

-- 별칭 한글x 공백x
SELECT city AS 도시, YEAR(sale_date) AS 연도, SUM(amount) AS '총 매출' -- 3) 특정 도시의 연도별 총 매출 집계 조회
...
SELECT ptype AS 'payment_type', AVG(amount) AS 'payment_average'
FROM payments
GROUP BY ptype;

-- 위 결과에 40,000원 이상인 데이터 구하기 => HAVING
SELECT ptype AS 'payment_type', AVG(amount) AS 'average'
FROM payments
-- WHERE AVG(amount) >= 40000 -- WHERE절은 GROUP BY나 집계 함수가 처리되기 전에 실행되기 때문
GROUP BY ptype
