...
CREATE TABLE products (
  id INTEGER AUTO_INCREMENT, -- 아이디(자동으로 1씩 증가),
  name VARCHAR(50) NOT NULL UNIQUE, -- 상품명(필수 입력, 고유키 지정) <-- 대체키
  price INTEGER NOT NULL CHECK (price > 0), -- 가격(필수입력, 양수만 허용)
  product_type VARCHAR(50) DEFAULT 'NONE', -- 상품 유형(기본값 NONE) <-- NULL을 넣으면 NULL이 들어감
  PRIMARY KEY (id), -- 기본키 지정: id
...
CREATE TABLE order_details (
  ...
  product_id INTEGER NOT NULL, -- 주문 아이디(필수 입력) <-- 필수적 관계
