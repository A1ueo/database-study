...
-- 안쪽서브쿼리의 실행 경과를 받아 바깥쪽 메인쿼리가 실행됨
...
SELECT
  ...
  amount AS '결제 금액'
  amount - (평균 결제 금액) AS '평균 결제 금액과의 차이'
FROM payments;
...
JOIN (
  ...
  FROM order_details
  GROUP BY product_id -- 개별 실행
) od ON p.id = od.product_id;

-- (참고) 다른 방법: 일단 붙여놓고 그룹화 및 집계
SELECT name AS '상품명', SUM(count) AS '주문 개수'
...
-- 상품 * 주문상세 조인해서 -> 상품명으로 그룹화 -> 상품별로 매출을 집계
-- 메인쿼리: 전체 상품의 매출을 조회 => 크림 치즈보다 매출이 높은 상품 조회로 변경
SELECT name AS '상품명', (price * SUM(count)) AS '매출'
FROM products p
...
FROM order_details od
WHERE product_id IN (
  -- 서브쿼리: 우유 식빵과 크림 치즈의 아이디를 반환(N * 1)
  SELECT id
  FROM products
...
-- user에 products를 바로 붙일 수 없으므로 orders와 order_details를 붙여 연결
...
-- 형식
컬럼명 비교_연산자 ANY (다중행의_단일_컬럼을_반환하는_서브쿼리);
...
-- 형식
컬럼명 비교_연산자 ALL (다중행의_단일_컬럼을_반환하는_서브쿼리);
...
WHERE NOT EXISTS (
  ...
  WHERE u.id = o.user_id AND payment_type = 'COCOA PAY'
);
