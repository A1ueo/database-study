...
7.1 조인이란
*/
-- 두 테이블로 나뉜 여러 정보를 하가로 연결하여 가져오는 명령
-- 테이블 간에 일치하는 컬럼을 기준으로 두 테이블을 하나로 합쳐 보여줌


-- 조인 컬럼(기준이 되는 컬럼):
-- 두 테이블이 동시에 가지고 있는 컬럼으로 조인하기 위해서 사용하는 컬럼
...
-- 부모 테이블 데이터 삭제시 자식 처리 정의 가능
-- 실수로 잘못된 참조값을 넣는 것을 방지

-- 조인하기
-- 특정 사진에 달린 모든 댓글의 닉네임과 본문을 조히하는 법?
USE stargram;
...
-- ON: 댓글 테이블의 외래키 = 사용자 테이블의 기본키가 같은 것 끼리
...
-- 6) 테이블명에 별칭 사용 가능
-- 간결한 쿼리 작성및 가독성 향상에 도움
-- 사용 예: comments 테이블과 users 테이블에 별칭 붙이기
SELECT body, nickname
FROM comments AS c JOIN users u ON c.user_id = u.id;
-- 참고: 테이블에 별칭을 붙일 때는 AS 거의 생략
