-- Active: 1749543480014@@127.0.0.1@3306@store
/*
5. 다양한 자료형 활용하기
5.1 자료형이란
*/
...
CREATE TABLE student_records (
    id INTEGER PRIMARY KEY, -- 아이디(표준_정수)
...
CREATE TABLE addresses (
    postal_code CHAR(5), -- 우편번호(고정 길이 문자: 5자), 주로 고정된 길이의 코드값(예: 국가코드)
...
-- 2) TEXT (참조 타입)
-- 컬럼 총합 한도(65535)가 있는데 참조 타입이기 때문에 영향을 받지 않음
-- 긴 문자열 저장을 위한 타입
...
-- (참고)
-- 자바 웹 개발을 포함한 대부분의 어플리케이션에서는 이미지나 동영상 같은 대용량 파일을
...
CREATE TABLE user_profiles (
    id INT PRIMARY KEY, -- 아이디(표준 정수) -- 기본키 지정: id
    email VARCHAR(255), -- 이메일(가변 길이 문자: 최대 255자)
    phone CHAR(13),     -- 전화번호(고정 길이 문자: 13자)
    introduce TEXT,     -- 자기소개(긴 문자열: 최대 64KB)
    picture MEDIUMBLOB,         -- 프로필 사진(파일: 최대 16MB)
    gender ENUM('남', '여') -- 성별(선택 목록 중 택 1)
);
DESC user_profiles;

INSERT INTO user_profiles (id, email, phone, introduce, picture, gender)
VALUES
    (1, 'geoblo@naver.com', '012-3456-7890', '안녕하십니까!', NULL, '남'), 
    (2, 'hongsoon@example.com', '098-7654-3210', '반갑습니다요!', NULL, '여');
...
-- DAY(날짜): 입력 날짜의 '일' 추출, 예: ('2024-04-15') => 15
-- EXTRACT(필드 FROM 날짜): 입력 날자에서 특정 '필드' 추출, 예: EXTRACT(YEAR FROM '2024-04-15') => 2024
...
-- 주문 시각이 2004-11-15부터 2024-12-15 사이에 있는 주문의 개수는?
...
-- WHERE name >= 'ㄱ' AND name < 'ㄴ'; -- 'ㄱ' 다음은 'ㄴ'이 아니라 'ㄲ'임
