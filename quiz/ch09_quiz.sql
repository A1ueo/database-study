...
-- 다른 방법: JOIN을 이용한 방법
SELECT name, COUNT(ep.project_id) AS project_count
FROM employees e
...
-- 다른 방법: 서브쿼리 없이
SELECT d.name, COUNT(department_id) AS 'employees_count' -- 직원이 없는 부서는 0으로 표시
FROM departments d
LEFT JOIN employees e ON d.id = e.department_id -- 직원이 없는 부서도
